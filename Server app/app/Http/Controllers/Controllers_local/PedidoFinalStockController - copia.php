...
class PedidoFinalStockController extends Controller
{
    ...
    {
        try {
            // 🔒 FILTRAR POR NEGOCIO ACTUAL - Solo ver su propio historial
            $appId = CurrentApp::getApp();
            
            \Log::info('🔍 getStockPorNegocio - Negocio: ' . $appId);
            
            $query = DB::connection('easyerp')->table('pedidofinal_stock_por_negocio')
                ->where('app_id', $appId); // 🔒 FILTRO POR NEGOCIO
            ...
            \Log::info('✅ Historial Stock - Total registros del negocio: ' . $registros->count());
            ...
            \Log::error('❌ Error en getStockPorNegocio: ' . $e->getMessage());
            return response()->json([
                'error' => 'Error al obtener historial',
                'message' => $e->getMessage()
            ], 500);
    ...
    {
        try {
            // 🔒 FILTRAR POR NEGOCIO ACTUAL - Solo ver su propio stock
            $appId = CurrentApp::getApp();
            
            \Log::info('🔍 getResumenStockPorNegocio - Negocio: ' . $appId);
            
            // Traer solo los registros del negocio actual
            $todosLosRegistros = DB::connection('easyerp')->table('pedidofinal_stock_por_negocio')
                ...
                ->where('pedidofinal_stock_por_negocio.app_id', $appId) // 🔒 FILTRO POR NEGOCIO
                ->select(
            ...
            \Log::info('🔍 Total registros del negocio ' . $appId . ': ' . $todosLosRegistros->count());
            
            // Agrupar en PHP para obtener el último por producto (ya filtrado por negocio)
            $resumen = [];
            $yaVistos = [];
            ...
            foreach ($todosLosRegistros as $registro) {
                ...
                // Solo agregar si no hemos visto este producto
                if (!isset($yaVistos[$productoId])) {
                    $resumen[] = $registro;
                    $yaVistos[$productoId] = true;
                    \Log::info("   ✅ Agregado: {$registro->producto_nombre}");
                }
            }

            \Log::info('✅ Resumen Stock - Total productos: ' . count($resumen));
