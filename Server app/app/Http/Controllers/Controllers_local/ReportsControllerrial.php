...
class ReportsController extends Controller
{
  ...
  public function getSellsByDayAndHour(Request $request)
  {
    $database2 = Config::get('database.connections.mysql_local.database');
    ...
    // Consulta SQL para obtener ventas agrupadas por día de la semana y hora
    $query = "
        SELECT 
    ...
    // Inicializar matriz (0-6 días, 6-23 horas)
    for ($day = 0; $day <= 6; $day++) {
        $heatmapData[$day] = [];
  ...
  public function getCounters(Request $request, $self = false)
  {
   ...
   // Inicializar el contador de tipos de ventas
    ...
    // 🚨 IMPORTANTE: Los métodos especiales (uber, junaeb, rappi, etc.) deben ir PRIMERO
    // para evitar que se confundan con boleta/efectivo cuando generan folios SII
    $ajustes = [
      'factura' => ['modulos.ventas.submodulos.sii.ajustes.factura', ['factura', 'Factura']],
      
      // Métodos especiales PRIMERO (antes de boleta/efectivo)
      'uber' => ['modulos.ventas.submodulos.sii.ajustes.uber', ['uber', 'uber_eats', 'Uber']],
      'rappi' => ['modulos.ventas.submodulos.sii.ajustes.rappi', ['rappi', 'Rappi']],
      'junaeb' => ['modulos.ventas.submodulos.sii.ajustes.junaeb', ['junaeb', 'Junaeb']],
      'pedidos_ya' => ['modulos.ventas.submodulos.sii.ajustes.pedidos_ya', ['pedidos_ya', 'Pedidos_ya']],
      ...
      'credito' => ['modulos.ventas.submodulos.sii.ajustes.credito', ['credito', 'Credito']],
      'transferencia' => ['modulos.ventas.submodulos.sii.ajustes.transferencia', ['transferencia', 'Transferencia']],
      'cheque' => ['modulos.ventas.submodulos.sii.ajustes.cheque', ['cheque', 'Cheque']],
      'banco' => ['modulos.ventas.submodulos.sii.ajustes.banco', ['banco', 'Banco']],
      'sodexo' => ['modulos.ventas.submodulos.sii.ajustes.sodexo', ['sodexo', 'Sodexo']],
      'multicaja' => ['modulos.ventas.submodulos.sii.ajustes.multicaja', ['multicaja', 'Multicaja']],
      'edenred' => ['modulos.ventas.submodulos.sii.ajustes.edenred', ['edenred', 'Edenred']],
      'convenio_empresa' => ['modulos.ventas.submodulos.sii.ajustes.convenio_empresa', ['convenio_empresa', 'Convenio_empresa']],
      'nota_de_credito' => ['modulos.ventas.submodulos.sii.ajustes.nota_de_credito', ['nota_de_credito', 'Nota_de_credito']],
      'guia_despacho' => ['modulos.ventas.submodulos.sii.ajustes.guia_despacho', ['guia_despacho', 'Guia_despacho']],
      
      // Boleta y efectivo al FINAL (para que no interfieran con métodos especiales)
      'boleta' => ['modulos.ventas.submodulos.sii.ajustes.boleta', ['boleta', 'Boleta']],
      'efectivo' => ['modulos.ventas.submodulos.sii.ajustes.boleta_local', ['boleta_local', 'efectivo', 'Efectivo', 'Boleta_local']],
    ];
    ...
    foreach ($ventas as $venta) {
      // 🚨 LOG CRÍTICO: Ver TODAS las ventas con sus campos de pago
      \Log::info('🔍 VENTA PROCESADA:', [
          ...
          'response_folio_other' => $venta->response_folio_other,
      ...
      // 🚨 CRÍTICO: Si ya se contó en un método de pago (uber, junaeb, etc.), NO procesar folios SII
      // Esto evita que Uber (que genera boleta SII) se sume también a "boleta" o "efectivo"
      if ($counted) {
          continue; // Saltar al siguiente registro sin procesar folios
      }

      // Procesar ventas sin SII
      if (!$venta->sell_folio && isset($_request['noSii'])) {
          $counters['noSii'] += $venta->total;
      } elseif ($venta->sell_folio) {
          $folio = Folio::where('sell_id', $venta->id)->first();
          if ($folio) {
              if (isset($_request['factura']) && $folio->type == 'factura') {
