...
<table class="body-texts-table">
    ...
    <tr>
      ...
      <td class="text-left fsDfl">{{$order['envs']->sii_giro_emisor->value}}</td>
    </tr>

    <tr>
      <td class="text-uppercase fs7">Direccion</td>
      <td class="text-left fsDfl">{{$order['envs']->sii_dirorigen->value}}</td>
    </tr>
  
    <tr>
      <td class="text-uppercase fs7">Comuna</td>
      <td class="text-left fsDfl">{{$order['envs']->sii_comuna_origen->value}}</td>
    </tr>
...
<div>
  ...
  <p class="text-uppercase fsDfl">
    Mesa: {{ $order['board']->name  }}
  </p>
    @if(!empty($order['client_ticket']))
      <p class="text-uppercase fsDfl mb-2">
...
<table class="body-texts-table">
      ...
      <?php foreach ($sell['products'] as $product) { ?>
        <tr>
          ...
          <?php
            ...
            $totales += $product->totalPrice;
          ?>
        </tr>
      <?php } ?>
    </table>
      <hr>
    <p class="text-spacing-3 text-right text-uppercase mb-1 mt-1">
      <?php
    ...
    <p class="text-spacing-3 header-title text-right text-uppercase mb-1 mt-1 fs12 text-bold">
        TOTAL: <span class="ml-3 fs12">${{ formatoChilenoBoleta($sell['total']) }}</span>
    </p>
