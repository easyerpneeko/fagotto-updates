  ...
  */
  public function newSell(Request $request, $order_id = false)
  {
    ...
    foreach ($items as $item) {
      ...
      // Añadir comentario/detalle adicional si existe (DscItem para SII)
  ...
  public function printPDF(Request $request, $id, $boleta = false, $self = false)
  {
    ...
    // Formatear el método de pago para que sea legible
    if (isset($sell['paymode'])) {
      $sell['paymode_formatted'] = $this->formatPaymentMethod($sell['paymode']);
    } elseif (isset($sell['other_type'])) {
      $sell['paymode_formatted'] = $this->formatPaymentMethod($sell['other_type']);
    } elseif (isset($sell['type'])) {
      $sell['paymode_formatted'] = $this->formatPaymentMethod($sell['type']);
    } else {
      $sell['paymode_formatted'] = 'No especificado';
    }
