...
async function getPedidos(startDate = null, endDate = null, page) {
    if (id) {
        ...
        if (startDate && endDate) {
            url = generarURLApi(`/web/getAppRequestsReposteria${params}&id=${id}&startDate=${startDate}&endDate=${endDate}`);
        } else {
            url = generarURLApi(`/web/getAppRequestsReposteria${params}&id=${id}`);
        }
...
function verPedido(app_id, id) {
    __conection({
        url: generarURLApi(`/local/request/view/reposteria/${app_id}/${id}`),
        header: credentials(),
        dev: true,
        method: 'GET'
...
function aprobarPedido(app_id, id) {
    ...
    __conection({
        url: generarURLApi(`/local/request/approve/reposteria/${app_id}/${id}`),
        header: credentials(),
        dev: true,
        method: 'PUT'
...
function mirarPedido(app_id, id) {
    ...
    __conection({
        url: generarURLApi(`/local/request/watch/reposteria/${app_id}/${id}`),
        header: credentials(),
        dev: true,
        method: 'PUT'
...
function rechazarPedido(app_id, id) {
    ...
    __conection({
        url: generarURLApi(`/local/request/decline/reposteria/${app_id}/${id}`),
        header: credentials(),
        dev: true,
        method: 'PUT'
...
async function loadSucursalesList() {
    ...
    try {
        await __conection(
            {
                url: generarURLApi(`/getAppsPedidosReposteria`),
                header: credentials(),
                dev: true,
                method: 'POST'
            },
    ...
    apps.forEach((app, index) => {
        ...
        if (app.pedidosNew > 0) {
            fila = `  <li class="nav-item list-group-item">
                            <a href="/pages/pedidos_reposteria.html?id=${app.id}" type="button" class="nav-link d-inline position-relative">
                                ${app.name} 
...
async function facturarPedido(){
    ...
    if(app_id && pedido_id && validateForm()){
        ...
        try {
            await __conection(
                {
                    url: generarURLApi(`/web/pedido/facturar/reposteria`),
                    header: credentials(),
                    dev: true,
                    method: 'POST',
                },
